...
match_id_required = 16215
home_team_required ="Real Betis"
...
for i,shot in shots.iterrows():
    ...
    if (team_name==home_team_required):
        if goal:
            ...
            if(shot['player_name']=='Lorenzo JesÃºs MorÃ³n GarcÃ­a'):
                plt.text((x+1),pitchWidthY-y+1,'Loren Moron') 
        else:
            shotCircle=plt.Circle((x,pitchWidthY-y),circleSize,color="red")     
        ...
        if goal:
            ...
            if(shot['player_name']=='Lionel AndrÃ©s Messi Cuccittini'):
                plt.text((pitchLengthX-x+1),y+1,'Lionel Messi')
            if(shot['player_name']=='Luis Alberto SuÃ¡rez DÃ­az'):
                plt.text((pitchLengthX-x+1),y+1,'Luis Suarez')
        else:
            shotCircle=plt.Circle((pitchLengthX-x,y),circleSize,color="blue")      
            shotCircle.set_alpha(.2)
...
#passes by Caroline Seger

passes = df.loc[df['type_name']== 'Pass'].set_index('id')
(fig,ax) = createPitch(pitchLengthX,pitchWidthY,'yards','grey')
for i,thepass in passes.iterrows():
    if thepass['player_name'] == 'Lionel AndrÃ©s Messi Cuccittini':
